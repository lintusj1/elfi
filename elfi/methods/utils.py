...
class OutputSift:
    ...
    def __init__(self, output_names, batch_size, discrepancy_name=None, max_sample_size=None):
        """

        Parameters
        ----------
        output_names : list
            First name must be the discrepancy by which the samples are sifted
        batch_size : int
        discrepancy_name : str, optional
            Specify the discrepancy name in output_names. Default is the first name in
            output_names.
        max_sample_size : int, optional
            Default is max(10000, batch_size)

        """
        self.output_names = list(output_names)
        self.discrepancy_name = discrepancy_name or self.output_names[0]
        self.batch_size = batch_size
        self.max_sample_size = max(max_sample_size or 10000, batch_size)
    ...
    def add_batch(self, batch, batch_index):
        ...
        # Add the indexes
        batch['_index'] = np.arange(batch_index,
        ...
        sort_mask = np.argsort(self.discrepancies, axis=0).ravel()
        for values in self._outputs.values():
            values[:] = values[sort_mask]

        self.n_batches += 1

    def get_threshold(self, sample_index=None):
        sample_index = self.max_sample_size - 1 if sample_index is None else sample_index
        return self.discrepancies[sample_index].item()

    def get_accept_rate(self, sample_size=None):
        sample_size = self.max_sample_size if sample_size is None else sample_size
        return min(1.0, sample_size / self.n_sim)

    def get_outputs(self, sample_size=None):
    ...
    def discrepancies(self):
        return self._outputs[self.discrepancy_name]
